...
function Bank.LoadStore(LoadedStore: string | table, Options: table?)
	...
	LoadedStore = (type(LoadedStore) == "string" or type(LoadedStore) == "table") and LoadedStore
		or Error.new("LoadedStore must be a string or table")
...
do
	...
	Global.AutosaveRefreshLocks = Settings.AutosaveCooldown <= (Settings.AssumeDeadSessionLock * 0.9)

	if not Global.AutosaveRefreshLocks then
		task.spawn(function()
			RefreshLocks()
			task.wait(Settings.AssumeDeadSessionLock * 0.9)
		end)
	end
end
